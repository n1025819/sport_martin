...
之後每天台灣時間 09:00、14:00、20:00、23:00 自動增量更新。請勿公開 `GARMIN_TOKEN_KEY`，也不要提交未加密的 `garmin_tokens.json`。
...
- 活動紀錄：單車 / 跑步 / 游泳 / 重訓分頁
- All Time 累計數據
- 每天 09:00 / 14:00 / 20:00 / 23:00（台灣時間）自動更新
...
> 首次跑完，`data/strava.json` 和 `data/itt-segments.json` 會被 commit 回 repo，之後每天台灣時間 09:00 / 14:00 / 20:00 / 23:00 自動增量更新。
...
```
台灣時間每天四次（09:00 / 14:00 / 20:00 / 23:00）
   ↓
...
```mermaid
flowchart TD
  Cron(["⏰ 台灣 09:00 / 14:00 / 20:00 / 23:00<br/>UTC 01:00 / 06:00 / 12:00 / 15:00"]) --> Token
